...
const CardEmpresa = memo(function CardEmpresa({ empresa, onEscolher }) {
  ...
  return (
    ...
    >
      <span
        ...
        style={{ background: `linear-gradient(to bottom, ${cor}, ${cor}40)` }}
      />
      <span
        aria-hidden="true"
        className="pointer-events-none absolute left-0 top-0 h-full w-1.5"
        style={{ background: `linear-gradient(to bottom, ${cor}, ${cor}aa)` }}
      />
      <span
        aria-hidden="true"
...
export default function CampoEmpresa({ empresas = [], navigate }) {
  const escolherEmpresa = useCallback((empresa) => {
