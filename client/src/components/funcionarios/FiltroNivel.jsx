...
function FiltroNivel({ setFuncionarioFiltro, funcionarioFiltro }) {
  const [open, setOpen] = useState(false);
  const btnRef = useRef(null);
  const popRef = useRef(null);
  const [pos, setPos] = useState({ top: 0, left: 0 });

  const niveis = ["Inicial", "Júnior", "Pleno", "Sênior"];

  function clica(nivel) {
    const valor = nivel.toLowerCase();
    const selecionado = funcionarioFiltro.some((s) => s.nivel_nome === valor);
                  ...
                  return (
                    <li key={nivel}>
                      ...
                      >
                        ...
                        <span className="block truncate">{nivel}</span>
                      </button>
                    </li>
                  );
                })}
