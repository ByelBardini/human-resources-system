...
const arquivosDir = path.join(uploadRoot, "notificacoes");

fs.mkdirSync(arquivosDir, { recursive: true });

function makeFilename(req, file, cb) {
  const ext = path.extname(file.originalname).toLowerCase();
  const baseFromBody = String(req.body?.nome || "notificacao")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
const uploadArquivoNotificacao = multer({
  storage,
  fileFilter,
  limits: { fileSize: 5 * 1024 * 1024 },
});

export default uploadArquivoNotificacao;
