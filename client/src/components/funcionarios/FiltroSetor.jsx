...
function FiltroSetor({
  ...
  const setores = [
    ...new Set(
      (funcionarios ?? [])
        .map((f) => f.setor?.setor_nome?.trim().toLowerCase())
        .filter(Boolean)
    ),
  ];

  function clica(setorNome) {
    const selecionado = funcionarioFiltro.includes(setorNome);

    setFuncionarioFiltro((prev) =>
      selecionado ? prev.filter((c) => c !== setorNome) : [...prev, setorNome]
    );
  }
          ...
          >
            ...
            >
              <ul className="space-y-1">
                {setores.map((setor) => {
                  const selecionado = funcionarioFiltro.includes(setor);
                  return (
                    <li key={setor}>
                      <button
                        type="button"
